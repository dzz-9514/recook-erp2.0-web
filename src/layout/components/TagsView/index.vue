...
<template>
  <div class="tags-container">
    <scroll-pane ref="scrollPaneRef" @scroll="handleScroll">
      <router-link v-for="tag in visitedViews" :key="tag.path" :class="'tags-item ' + (isActive(tag) ? 'active' : '')" :data-path="tag.path"
        ...
        {{ translateRouteTitleI18n(tag.meta?.title) }}
          ...
          <el-image v-if="!isAffix(tag)" class="tags-item-close" @click.prevent.stop="closeSelectedTag(tag)" style="width:16px;height:16px" src="https://akustatic.oss-cn-hangzhou.aliyuncs.com/card/close.png"></el-image>
      </router-link>
    </scroll-pane>
